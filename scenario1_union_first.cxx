...
void run_benchmark(const std::vector<std::string> &primaryPaths,
  ...
  for (unsigned i = 0; i < primaryPaths.size(); ++i) {
    primaryNTuples.emplace_back("ntuple", primaryPaths[i]);
  ...
  if (debug) {
    hist->DrawClone("SAME");
    canvas->SaveAs("scenario1_union_first.png");
  }
}
