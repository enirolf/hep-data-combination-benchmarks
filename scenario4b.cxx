...
void run_benchmark(const std::vector<std::string> &mainSamplePaths,
  ...
  std::unique_ptr<RNTupleProcessor> mainProcessor = RNTupleProcessor::CreateChain(mainNTuples);
  std::vector<std::unique_ptr<RNTupleProcessor>> auxProcessors;
  auxProcessors.push_back(RNTupleProcessor::CreateChain(auxNTuples));

  auto processor = RNTupleProcessor::CreateJoin(std::move(mainProcessor), auxProcessors, {});
  ...
  auto hist = std::make_unique<TH1D>("scenario4b", "scenario4b", 64, -8, 8);
...
int main(int argc, char *argv[]) {
  ...
  for (unsigned i = 0; i < N_SAMPLES; ++i) {
    mainSamplePaths.emplace_back("data/scenario4/" + nEvents +
                                 "_evts_primary_sample" + std::to_string(i) +
                                 ".root");
    auxSamplePaths.emplace_back("data/scenario4/" + nEvents +
